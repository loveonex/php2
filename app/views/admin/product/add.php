<form class='col-md-5' action="/admin/product/add" method="post" enctype="multipart/form-data">
    ...
    <div class='form-group'>
        ...
        <select class="form-control" name="cate_id">
            <?php foreach ($categories as $category) : ?>
                <option value="<?= $category->id ?>" <?= isset($request['cate_id'])  ? ($request['cate_id'] == $category->id ? 'selected' : '') : '' ?>><?= $category->name ?></option>
            <?php endforeach ?>
        </select>
    </div>
    <br>
    ...
    <?php if (isset($errors['description'])) : ?>
        ...
        <?php endif ?>
        <label>Mô tả</label>
        <textarea name="description" class="form-control" id="" cols="30" rows="10"><?= $request['description'] ?? '' ?></textarea>
    </div>
    <br>
    <div class='form-group'>
